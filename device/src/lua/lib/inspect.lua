...
-- Beep: added the no_newlines feature

local inspect ={}
inspect.__VERSION = '1.2.0-beep'
...
function Inspector:new(t, depth, no_newlines)
  local inspector = {
    buffer = {},
    depth = depth,
...
function Inspector:putValue(v)
  ...
  -- note(shawn): this is a hack to avoid printing strings that actually
  -- represent data, since they screw up my tmux session. Of course it
  -- doesn't actually check that long strings represent data.
  if tv == 'string' and string.len(v) < 1024 then
    self:puts(smartQuote(unescape(v)))
  elseif tv == 'string' then
    self:puts(smartQuote('<too long>'))
...
setmetatable(inspect, { __call = function(_,t,depth, no_newlines)
  return Inspector:new(t, depth, no_newlines):tostring()
end })
