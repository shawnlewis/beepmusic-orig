...
// shawn; 25kB/s
static int		max_rte = 1000 * 1024;
static int		sqns = 2000;
...
/* dispatch loop */
	...
	do {
		...
		const int status = pgm_recv (nak_sock, buf, sizeof(buf), MSG_DONTWAIT, NULL, &pgm_err);
		switch (status) {
...
/* parse program arguments */
	...
	if (create_sock() && create_nak_thread())
	{
#define SIZE 1400
        int sent_data = 0;
        char* buf[SIZE];
        memset(buf, 0xa5, SIZE);
        while (sent_data < 100 * 1024 *  1024) {
            //const int status = pgm_send (sock, argv[optind], strlen (argv[optind]) + 1, NULL);
            const int status = pgm_send (sock, buf, SIZE, NULL);
                if (PGM_IO_STATUS_NORMAL != status) {
                fprintf (stderr, "pgm_send() failed.\n");
                }
            if (sent_data % 100000 == 0) {
                printf("sent data: %d\n", sent_data);
            }
            sent_data += SIZE;
            usleep(1 / (200.0 * 1024 / 1500) * 1000000);
        }
	}
