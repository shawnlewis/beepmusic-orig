...
static uint8_t sin_lut[256] = \
        {128, 131, 134, 137, 140, 143, 146, 149, 152, 155, 158, 162, 165, 167,
         ...
         59, 62, 65, 67, 70, 73, 76, 79, 82, 85, 88, 90, 93, 97, 100, 103, 106,
         109, 112, 115, 118, 121, 124};

#define index(x) ((x) >= 0 ? ((x) % 256) : (256 + ((x) % 256)))

static inline int lookup_sin(int x) {
    return sin_lut[index(x)] - 128;
}

static inline int lookup_cos(int x) {
    return sin_lut[index(x + 192)] - 128;
}

static int lua_trigint_sin(lua_State *L) {
    int x = luaL_checkinteger(L, 1);

    lua_pushnumber(L, lookup_sin(x));
    return 1;
}

static int lua_trigint_cos(lua_State *L) {
    int x = luaL_checkinteger(L, 1);

    lua_pushnumber(L, lookup_cos(x));
    return 1;
}

static luaL_reg trigint_func[] = {
    {"sin", lua_trigint_sin},
    {"cos", lua_trigint_cos},
    {NULL, NULL}
};

int luaopen_trigint(lua_State *L) {
    luaL_openlib(L, "trigint", trigint_func, 0);
