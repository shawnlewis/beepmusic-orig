...
char *get_attribute_value(struct xmlelement *element, const char *name) {
    IXML_Element *el = to_ielem(element);
    const char *value = (el != NULL
                  ? ixmlElement_getAttribute(el, name)
                  : NULL);
