...
inline void control_service_lock(void) {
    service_lock();
}

inline void control_service_unlock(void) {
    service_unlock();
}
...
void change_volume_level(int level) {
    // level is [0, 100]
    const float decibel = volume_level_to_decibel(level);

    char volume[10];
    snprintf(volume, sizeof(volume), "%d", level);
...
static int set_volume_db(struct action_event *event) {
    ...
    int level = volume_decibel_to_level(decibel);
    output_set_volume(level / 100.0);
    service_unlock();
...
static int set_volume(struct action_event *event) {
    ...
    const double fraction = volume_level / 100.0;
