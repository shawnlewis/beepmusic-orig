...
inline void transport_service_lock(void) {
    service_lock();
}

inline void transport_service_unlock(void) {
    service_unlock();
}
...
static int set_avtransport_uri(struct action_event *event)
{
    ...
    output_set_uri(uri, meta,
            (requires_meta_update
...
// user_thread: true if calling this from a non gmrender thread.
void inform_play_transition_from_output(enum PlayFeedback fb, bool user_thread) {
    if (user_thread) {
        service_lock();
    }
    switch (fb) {
