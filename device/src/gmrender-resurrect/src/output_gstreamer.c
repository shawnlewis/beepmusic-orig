...
static void MetaModify_add_tag(const GstTagList *list, const char *tag,
                   gpointer user_data) {
...
static bool my_bus_callback(GstBus * bus, GstMessage * msg,
                gpointer data)
{
...
static int output_gstreamer_set_mute(int m) {
    ...
    g_object_set(player_, "mute", (bool) m, NULL);
    return 0;
}
