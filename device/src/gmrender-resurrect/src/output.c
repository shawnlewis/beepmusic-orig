...
void output_set_device(struct upnp_device *device) {
    if (output_module->set_device) {
        return output_module->set_device(device);
    }
}

void output_set_uri(const char *uri, const char *meta,
        output_update_meta_cb_t meta_cb) {
    if (output_module && output_module->set_uri) {
        output_module->set_uri(uri, meta, meta_cb);
    }
}
...
void output_get_name_and_usn(char **name, char **usn) {
    output_module->get_name_and_usn(name, usn);
}
