                                              ...
                                              *sr_event)
{
    ...
    // NOTE(shawn): This should really delegate to the services themselves
    // to figure out what values to return.

    if (strstr(sr_event->ServiceId, "RenderingControl")
            || strstr(sr_event->ServiceId, "AVTransport")) {
    ...
    } else if (strstr(sr_event->ServiceId, "ConnectionManager")) {
        const char **eventvar_names = malloc(sizeof(char *) * (var_count + 1));
        const char **eventvar_values = malloc(sizeof(char *) * (var_count + 1));
        ...
        for (int i = 0; i < var_count; ++i) {
            ...
            if (name && value
                    && strncmp("A_ARG_TYPE_",
                        name, strlen("A_ARG_TYPE_")) != 0) {
                eventvar_names[output_index] = name;
                eventvar_values[output_index] = value;
                output_index++;
            }
        }
        ithread_mutex_unlock(srv->service_mutex);

        eventvar_names[output_index] = NULL;
        eventvar_values[output_index] = NULL;

        rc = UpnpAcceptSubscription(priv->device_handle,
                        sr_event->UDN, sr_event->ServiceId,
                        eventvar_names, eventvar_values, output_index,
                        sr_event->Sid);
        if (rc == UPNP_E_SUCCESS) {
            result = 0;
        } else {
        ...
        LOG_ERROR(log_beep_main, "Unknown service for subscription: %s",
                sr_event->ServiceId);
    }
...
static bool initialize_device(struct upnp_device *result_device,
...
bool upnp_device_register(struct upnp_device *device)
{
    char *buf;
    int rc;
    ...
    if (device->registered) {
        int rc;
        rc = UpnpUnRegisterRootDevice(device->device_handle);
        if (UPNP_E_SUCCESS != rc) {
            Log_error("unpp", "UpnpUnRegisterRootDevice() Error: %s (%d)",
                  UpnpGetErrorMessage(rc), rc);
            return false;
        }
    }

    buf = upnp_create_device_desc(device->upnp_device_descriptor);
    rc = UpnpRegisterRootDevice2(UPNPREG_BUF_DESC,
                     buf, strlen(buf), 1,
                     &event_handler, device,
    ...
    rc = UpnpSendAdvertisement(device->device_handle, 100);
    if (UPNP_E_SUCCESS != rc) {
...
// HACK: These are global so we're assuming only one upnp_device will be
// used in the program.
char *cur_friendly_name = NULL;
char *cur_udn = NULL;

void upnp_device_set_name(const char *friendly_name,
        const char *udn,
        struct upnp_device *device) {
    if (device->upnp_device_descriptor->friendly_name) {
        free(device->upnp_device_descriptor->friendly_name);
    }
    device->upnp_device_descriptor->friendly_name = strdup(friendly_name);

    if (device->upnp_device_descriptor->udn) {
        free(device->upnp_device_descriptor->udn);
    }
    device->upnp_device_descriptor->udn = strdup(udn);

    if (device->registered) {
        // reregister it, otherwise we haven't been initialized yet, let the
        // user do that.
        upnp_device_register(device);
    }
}
                     ...
                     unsigned short port)
{
    ...
    result_device->registered = false;
    ...
    if (!initialize_device(result_device, ip_address, port)) {
        UpnpFinish();
        free(result_device);
        return NULL;
    }
