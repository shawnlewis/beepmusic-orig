...
static struct upnp_device *upnp_device_;
...
void register_mime_type(const char *mime_type) {
    ...
    if (strcmp("audio/x-aac", mime_type) == 0) {
    ...
    if (strcmp("audio/x-flac", mime_type) == 0) {
      register_mime_type_internal("audio/flac");
    }
    if (strcmp("audio/x-ogg", mime_type) == 0) {
      register_mime_type_internal("audio/ogg");
    }
...
int connmgr_init(struct upnp_device *device) {
    struct mime_type *entry;
    char *buf = NULL;
    char *p;
    int offset;
    ...
    if (buf == NULL) {
        LOG_ERROR(log_beep_main, "%s: initial malloc failed\n",
            __FUNCTION__);
        return -1;
    }
    ...
    for (entry = supported_types; entry; entry = entry->next) {
        ...
        if (buf == NULL) {
            LOG_ERROR(log_beep_main, "%s: realloc failed\n",
                __FUNCTION__);
            return -1;
        }
        p = buf;
...
{
    upnp_append_variable(event, CONNMGR_VAR_CUR_CONN_IDS, "CurrentConnectionIDs");
    return 0;
}

static int prepare_for_connection(struct action_event *event) {
    char connection_id_s[10];

    connection_id++;
    snprintf(connection_id_s, sizeof(connection_id_s), "%d", connection_id);

    // Update both our list of connection ids (we only ever have one),
    // and our current connection.
    VariableContainer_change(state_variables_,
            CONNMGR_VAR_CUR_CONN_IDS, connection_id_s);
    VariableContainer_change(state_variables_,
            CONNMGR_VAR_AAT_CONN_ID, connection_id_s);

    char *protocol_info = upnp_get_string(event, "RemoteProtocolInfo");
    if (!protocol_info) {
        return -1;
    }
    VariableContainer_change(state_variables_,
            CONNMGR_VAR_AAT_PROTO_INFO, protocol_info);

    // Send a notification that we have a new connection.
    ...
    eventvar_values[0] = connection_id_s;
    upnp_device_notify(upnp_device_, connmgr_service_.service_id,
            eventvar_names, eventvar_values, 1);

    upnp_append_variable(event, CONNMGR_VAR_AAT_CONN_ID, "ConnectionID");
    upnp_append_variable(event, CONNMGR_VAR_AAT_AVT_ID,  "AVTransportID");
    upnp_append_variable(event, CONNMGR_VAR_AAT_RCS_ID, "RcsID");
    return 0;
}

static int connection_complete(struct action_event *event) {
    const char *eventvar_names[] = {
        "CurrentConnectionIDs",
        NULL
    };
...
struct service *upnp_connmgr_get_service(void) {
    if (connmgr_service_.variable_container == NULL) {
        ...
        state_variables_ = connmgr_service_.variable_container;
    }
...
static struct action connmgr_actions[] = {
    ...
    [CONNMGR_CMD_CONNECTIONCOMPLETE] =    {"ConnectionComplete", connection_complete},    /* optional */
