...
void SongMetaData_clear(struct SongMetaData *value) {
    ...
    value->genre = NULL;
    free((char*)value->album_art_uri);
    value->album_art_uri = NULL;
    free((char*)value->mime_type);
...
int SongMetaData_parse_DIDL(struct SongMetaData *object, const char *xml) {
    ...
    value_node = find_element_in_element(item_node, "upnp:albumArtURI");
    if (value_node) object->album_art_uri = get_node_value(value_node);

    value_node = find_element_in_element(item_node, "res");
    if (value_node) {
        char *protocol_info =
                get_attribute_value(value_node, "protocolInfo");
        char *pos1 = NULL, *pos2 = NULL, *pos3 = NULL;
        pos1 = strchr(protocol_info, ':');
        if (pos1) {
            pos2 = strchr(pos1 + 1, ':');
            if (pos2) {
                pos3 = strchr(pos2 + 1, ':');
        ...
        if (pos1 && pos2 && pos3) {
            size_t len = pos3 - pos2 - 1;

            char *mime_type = malloc(len + 1);
            memcpy(mime_type, pos2 + 1, len);
            mime_type[len] = '\0';
