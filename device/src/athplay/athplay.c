#define here()        do { if (dbg) {printf("i2s: %s:%d\n", __func__, __LINE__); }} while(0)
/*
...
char *audev = "/dev/i2s";
int audio, bufsz, fine, recorder = 0;
...
playwav (int fd)
{
    ...
    printf("Settings bits per sample: %hu\n", hdr.bit_p_spl);
	if (ioctl(audio, I2S_DSIZE, hdr.bit_p_spl) < 0) {
		perror("I2S_DSIZE");
	} else {
        printf("Did set bits per sample\n");
    }

    printf("Settings sample frequency: %u\n", hdr.sample_fq);
	if (ioctl(audio, I2S_FREQ, hdr.sample_fq) < 0) {
		perror("I2S_FREQ");
	} else {
        printf("Did set sample frequency\n");
    }
