...
typedef int32_t fft_fixed;

#define FIXED_ONE 0x10000
#define FIXED_FRAC_BITS 16

static inline int32_t fixed_to_s32(fft_fixed x) {
    return x >> 16;
}

static inline fft_fixed int32_to_fixed(int32_t x) {
    return x << 16;
}
...
static inline fft_fixed fixed_mul(fft_fixed x, fft_fixed y) {
    register int32_t __hi, __lo, __result;
    asm(
        "smull %0, %1, %3, %4\n\t"
        "movs %0, %0, lsr #16\n\t"
        "adc %2, %0, %1, lsl #16"
...
static inline fft_fixed fixed_div(fft_fixed x, fft_fixed y) {
    int64_t z = ((int64_t)x << 32);
    return (int32_t) ((z / y) >> 16);
}
