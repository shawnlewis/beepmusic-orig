...
//static uint32_t pcm_sample_rates[] = {
//    11025, 22050, 32000, 44100, 48000, 8000, 12000, 16000, 24000, 96000, 88200, 176400, 192000
//};


/* Indexed by pcm_sample_size. Width in bytes. */
static uint32_t pcm_sample_widths[] = {
    1, 2, 3, 4
};


static sample_t pcm_read8bit(uint8_t *pos) {
    // 8 bit wav stores samples as unsigned char
    return (sample_t) ((((uint8_t)*pos) - 128) << 24);
}

static sample_t pcm_read16bitBE(uint8_t *pos) {
    sample_t sample = *pos << 24;
    sample |= *++pos << 16;
    return sample;
}

static sample_t pcm_read16bitLE(uint8_t *pos) {
    sample_t sample = *pos << 16;
    sample |= *++pos << 24;
    return sample;
}

static sample_t pcm_read24bitBE(uint8_t *pos) {
    sample_t sample = *pos << 24;
    sample |= *++pos << 16;
    sample |= *++pos << 8;
    return sample;
...
static sample_t pcm_read24bitLE(uint8_t *pos) {
...
static sample_t pcm_read32bitBE(uint8_t *pos) {
    sample_t sample = *pos << 24;
    sample |= *++pos << 16;
    sample |= *++pos << 8;
    sample |= *++pos;
...
static sample_t pcm_read32bitLE(uint8_t *pos) {
    /* XXX should this not increment pos? */
    return *((sample_t *)(void *)pos);
}


typedef sample_t (*pcm_read_func_t)(uint8_t *pos);
...
static bool decode_pcm_callback(void *data) {
...
static void *decode_pcm_start(uint8_t *params, uint32_t num_params) {
    ...
    */
    ...
    self->read_buffer = malloc(sizeof(uint8_t) * BLOCKSIZE);
