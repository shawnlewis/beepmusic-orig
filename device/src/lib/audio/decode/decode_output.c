...
static uint32_t decode_transition_type = 0;
static uint32_t decode_transition_period = 0;

static bool crossfade_started;
static size_t crossfade_ptr;
static fft_fixed transition_gain;
static fft_fixed transition_gain_step;
static uint32_t transition_sample_step;
static uint32_t transition_samples_in_step;
...
/* Polarity inversion */
static int32_t track_inversion[2] = { 1, 1 };

/* Output channels */
static uint8_t output_channels = 0;
...
static bool upload_samples(sample_t *buffer, uint32_t nsamples) {
    ssize_t n, len;
...
void decode_output_end(void) {
    ...
    decode_audio->written_track_samples = 0;
    decode_audio->sync_timestamp = 0;
}
...
void decode_output_samples(sample_t *buffer, uint32_t nsamples, int sample_rate) {
    size_t bytes_out;
    ...
    if (decode_first_buffer) {
        ...
        if (decode_transition_type & TRANSITION_CROSSFADE) {
            ...
            if (interval) {
                ...
                /* Gain steps */
                transition_gain_step = fixed_div(FIXED_ONE, fixed_mul(interval, int32_to_fixed(TRANSITION_STEPS_PER_SECOND)));
                transition_gain = 0;
            ...
            /* Gain steps */
            transition_gain_step = fixed_div(FIXED_ONE, int32_to_fixed(decode_transition_period * TRANSITION_STEPS_PER_SECOND));
            transition_gain = 0;
            transition_sample_step = sample_rate / TRANSITION_STEPS_PER_SECOND;
            transition_samples_in_step = 0;
        }
...
bool decode_output_can_write(uint32_t buffer_size, uint32_t sample_rate) {
    size_t freebytes;
...
void decode_output_set_transition(uint32_t type, uint32_t period) {
    if (!period) {
...
void decode_output_set_track_gain(uint32_t replay_gain) {
...
void decode_set_track_polarity_inversion(uint8_t inversion) {
...
void decode_set_output_channels(uint8_t channels) {
