...
void decode_init_buffers(void *buf, bool prio_inherit) {
    decode_audio = buf;
...
bool decode_check_start_point(void) {
    ...
    decode_audio->written_track_samples = BYTES_TO_SAMPLES(track_start_offset) +
        decode_audio->start_at_elapsed_samples;
    decode_audio->start_at_elapsed_samples = 0;
    decode_audio->sync_timestamp = 0; /* bug 15344: don't send previous-track data */

    return true;
}


static inline int16_t s16_clip(int16_t a, int16_t b) {
    int32_t s = a + b;
            ...
            int output_sample_rate)
{
            ...
            framesPerBuffer -= effects_frames)
    {
        ...
        if (sample_width == 24) {
            int32_t *output_ptr  = (sample_t *)outputBuffer;
            ...
            int16_t *output_ptr  = (int16_t *)outputBuffer;
