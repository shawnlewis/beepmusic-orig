...
static void debug_pagefaults()
{
    ...
    if (majflt > 0 || minflt > 0) {
        LOG_WARN(log_beep_main, "Pagefaults, Major:%ld Minor:%ld", majflt, minflt);
    }
                  ...
                  bool copyright)
{
    ...
    if ((err = snd_hctl_elem_read(state->iec958_elem, control)) < 0) {
        LOG_ERROR(log_beep_main, "snd_hctl_elem_read error: %s", snd_strerror(err));
        return;
    }
    ...
    LOG_DEBUG(log_beep_main, "iec958 status: %02x %02x %02x %02x",
          iec958.status[0], iec958.status[1], iec958.status[2], iec958.status[3]);

    if ((err = snd_hctl_elem_write(state->iec958_elem, control)) < 0) {
        LOG_ERROR(log_beep_main, "snd_hctl_elem_write error: %s", snd_strerror(err));
        return;
    }
}
...
static void playback_callback(struct decode_alsa *state,
        ...
        )
    {
        ...
        LOG_INFO(log_beep_main, "Starting audio: %" PRIu64, now);
        ...
        } else if (decode_audio->start_at_jiffies > now && now > decode_audio->start_at_jiffies - 9999) {
    ...
    while (decode_frames) {
        ...
        if (PCM_SAMPLE_WIDTH() == 24) {
            if (state->format == SND_PCM_FORMAT_S24_LE) {
                ...
                output_ptr = (int32_t *)(void *)output_buffer;
                ...
                output_ptr = (uint8_t *)(void *)output_buffer;
                decode_ptr = (sample_t *)(void *)(decode_fifo_buf + decode_audio->fifo.rptr);
                while (frames_cnt--) {
             ...
             snd_pcm_uframes_t output_frames)
{
    ...
    if (frames < 0) {
        LOG_WARN(log_beep_main, "overrun? %s", snd_strerror(frames));
...
static int pcm_close(struct decode_alsa *state, snd_pcm_t **pcmp, int mode) {
    ...
    if ((err = snd_pcm_drain(*pcmp)) < 0) {
        LOG_ERROR(log_beep_main, "snd_pcm_drain error: %s", snd_strerror(err));
    }

    if ((err = snd_pcm_close(*pcmp)) < 0) {
        LOG_ERROR(log_beep_main, "snd_pcm_close error: %s", snd_strerror(err));
    }
             ...
             uint32_t sample_rate)
{
    ...
    if ((err = snd_pcm_open(pcmp, device, mode, 0)) < 0) {
        LOG_ERROR(log_beep_main, "Playback open error: %s", snd_strerror(err));
        return err;
    }
    ...
    if ((err = snd_pcm_hw_params_any(*pcmp, hw_params)) < 0) {
        LOG_ERROR(log_beep_main, "hwparam init error: %s", snd_strerror(err));
        return err;
    }

    /* set hardware resampling */
    if ((err = snd_pcm_hw_params_set_rate_resample(*pcmp, hw_params, 1)) < 0) {
        LOG_ERROR(log_beep_main, "Resampling setup failed: %s", snd_strerror(err));
        return err;
    }

    /* set mmap interleaved access format */
    if ((err = snd_pcm_hw_params_set_access(*pcmp, hw_params, access)) < 0) {
        LOG_ERROR(log_beep_main, "Access type not available: %s", snd_strerror(err));
        return err;
    }
    ...
    if ((err = snd_pcm_hw_params_set_format(*pcmp, hw_params, state->format)) < 0) {
        ...
        if (state->format == SND_PCM_FORMAT_S24_LE) {
            ...
            LOG_INFO(log_beep_main, "S24_LE unavailable trying S24_3LE");
            err = snd_pcm_hw_params_set_format(*pcmp, hw_params, state->format);
        }

        if (err < 0) {
            LOG_ERROR(log_beep_main, "Sample format not available: %s", snd_strerror(err));
            return err;
        }
    }
    ...
    if ((err = snd_pcm_hw_params_set_channels(*pcmp, hw_params, 2)) < 0) {
        LOG_ERROR(log_beep_main, "Channel count not available: %s", snd_strerror(err));
        return err;
    }
    ...
    if ((err = snd_pcm_hw_params_set_rate_near(*pcmp, hw_params, &val, 0)) < 0) {
        LOG_ERROR(log_beep_main, "Rate not available: %s", snd_strerror(err));
        return err;
    }
    ...
    if ((err = snd_pcm_hw_params_set_periods_near(*pcmp, hw_params, &val, 0)) < 0) {
        LOG_ERROR(log_beep_main, "Unable to set period size %s", snd_strerror(err));
        return err;
    }
    ...
    if ((err = snd_pcm_hw_params_set_buffer_time_near(*pcmp, hw_params, &val, &dir)) < 0) {
        LOG_ERROR(log_beep_main, "Unable to set  buffer time %s", snd_strerror(err));
        return err;
    }

    /* set hardware parameters */
    if ((err = snd_pcm_hw_params(*pcmp, hw_params)) < 0) {
        LOG_ERROR(log_beep_main, "Unable to set hw params: %s", snd_strerror(err));
        return err;
    }

    if ((err = snd_pcm_hw_params_get_period_size(hw_params, &size, 0)) < 0) {
        LOG_ERROR(log_beep_main, "Unable to get period size: %s", snd_strerror(err));
        return err;
    }
    ...
    if ((err = snd_hctl_open(&state->hctl, state->playback_device, 0)) < 0) {
        LOG_ERROR(log_beep_main, "snd_hctl_open failed: %s", snd_strerror(err));
        goto skip_iec958;
    }

    if ((err = snd_hctl_load(state->hctl)) < 0) {
        LOG_ERROR(log_beep_main, "snd_hctl_load failed: %s", snd_strerror(err));
        goto skip_iec958;
    }
...
static int pcm_open(struct decode_alsa *state, bool loopback, int mode)
{
    int err;
    ...
    if (mode == SND_PCM_STREAM_PLAYBACK) {
        ...
        if (sample_rate == 0) {
            LOG_ERROR(log_beep_main, "invalid sample rate\n");
            sample_rate = 44100;
        }
...
static int pcm_test(struct decode_alsa *state) {
    ...
    LOG_DEBUG(log_beep_main, "max sample rate %d", decode_audio->max_rate);
...
static void *audio_thread_execute(void *data) {
    ...
    while (1) {
        ...
        if (do_open) {
            bool loopback = ((decode_audio->state & DECODE_STATE_LOOPBACK) != 0);
            ...
            if ((err = pcm_open(state, loopback, SND_PCM_STREAM_PLAYBACK)) < 0) {
                LOG_ERROR(log_beep_main, "Playback open failed: %s", snd_strerror(err));
                goto thread_error;
            }

            if (state->capture_device && loopback) {
                if ((err = pcm_open(state, loopback, SND_PCM_STREAM_CAPTURE)) < 0) {
                    LOG_ERROR(log_beep_main, "Capture open failed: %s", snd_strerror(err));
                    goto thread_error;
                }
            }
            else {
        ...
        if (pcm_state == SND_PCM_STATE_XRUN) {
            ...
            LOG_WARN(log_beep_main, "underrun!!! (at least %.3f ms long)", diff.tv_sec * 1000.0 + diff.tv_usec / 1000.0);

            if ((err = snd_pcm_recover(state->pcm, -EPIPE, 1)) < 0) {
                LOG_ERROR(log_beep_main, "XRUN recovery failed: %s", snd_strerror(err));
            }
            first = 1;
        }
        else if (pcm_state == SND_PCM_STATE_SUSPENDED) {
            if ((err = snd_pcm_recover(state->pcm, -ESTRPIPE, 1)) < 0) {
                LOG_ERROR(log_beep_main, "SUSPEND recovery failed: %s", snd_strerror(err));
            }
        }

        avail = snd_pcm_avail_update(state->pcm);
        if (avail < 0) {
            LOG_WARN(log_beep_main, "xrun (avail_update)");
            if ((err = snd_pcm_recover(state->pcm, avail, 1)) < 0) {
                LOG_ERROR(log_beep_main, "Avail update failed: %s", snd_strerror(err));
            }
            first = 1;
        }
        ...
        if (( err = snd_pcm_status(state->pcm, status)) < 0) {
            LOG_ERROR(log_beep_main, "snd_pcm_status err=%d", err);
        }
        ...
        if (avail < state->period_size) {
            if (first) {
                ...
                if (snd_pcm_state(state->pcm) != SND_PCM_STATE_RUNNING) {
                    if ((err = snd_pcm_start(state->pcm)) < 0) {
                        LOG_ERROR(log_beep_main, "snd_pcm_start error: %s", snd_strerror(err));
                    }
                }

                if (state->capture_pcm) {
                    if ((err = snd_pcm_start(state->capture_pcm)) < 0) {
                        LOG_ERROR(log_beep_main, "snd_pcm_start capture error: %s", snd_strerror(err));
                    }
                }
            }
            else {
                if ((err = snd_pcm_wait(state->pcm, 500)) < 0) {
                    LOG_WARN(log_beep_main, "xrun (snd_pcm_wait)");
                    if ((err = snd_pcm_recover(state->pcm, avail, 1)) < 0) {
                        LOG_ERROR(log_beep_main, "PCM wait failed: %s", snd_strerror(err));
                    }
                    first = 1;
                }
        ...
        while (size > 0) {
            ...
            if (state->has_mmap) {
                if ((err = snd_pcm_mmap_begin(state->pcm, &areas, &offset, &frames)) < 0) {
                    LOG_WARN(log_beep_main, "xrun (snd_pcm_mmap_begin)");
                    if ((err = snd_pcm_recover(state->pcm, err, 1)) < 0) {
                        LOG_ERROR(log_beep_main, "mmap begin failed: %s", snd_strerror(err));
                    }
                    first = 1;
                }
            }
            ...
            if (state->has_mmap) {
                buf = ((uint8_t *)areas[0].addr) + (areas[0].first + offset * areas[0].step) / 8;
            } else {
                if (!buf)
            ...
            if (state->flags & FLAG_STREAM_PLAYBACK) {
                ...
                if (state->capture_pcm) {
                    ...
                    if (size == ( snd_pcm_uframes_t) state->period_size) {
                        ...
                        decode_audio->sync_played_samples = decode_audio->written_track_samples;
                        // SHAWN: snd_pcm_status_get_delay always returns 0
                        // for some reason. snd_pcm_delay works though.
                        //delay = snd_pcm_status_get_delay(status);
                        snd_pcm_delay(state->pcm, (snd_pcm_sframes_t*) &delay);

                        if (decode_audio->sync_played_samples > delay) {

                            decode_audio->sync_played_samples -= delay;
                        }
                        ...
                        /* have not reached start of track yet */
                             ...
                             */
                            decode_audio->sync_played_samples = 0;
                        }

                        decode_audio->sync_timestamp = beep_millis();
                    }
            ...
            if (state->has_mmap) {
                ...
                if (commitres < 0 || (snd_pcm_uframes_t)commitres != frames) {
                    LOG_WARN(log_beep_main, "xrun (snd_pcm_mmap_commit) err=%ld", commitres);
                    if ((err = snd_pcm_recover(state->pcm, commitres, 1)) < 0) {
                        LOG_ERROR(log_beep_main, "mmap commit failed: %s", snd_strerror(err));
                    }
                    first = 1;
                }
            } else {
                ...
                if (commitres < 0 || (snd_pcm_uframes_t)commitres != frames) {
                    LOG_WARN(log_beep_main, "xrun (snd_pcm_writei) err=%ld", commitres);
                    if ((err = snd_pcm_recover(state->pcm, commitres, 1)) < 0) {
                        LOG_ERROR(log_beep_main, "sound write failed: %s", snd_strerror(err));
                    }
                    first = 1;
                }
            }
...
int main(int argv, char **argc)
{
    ...
    log_beep_main = LOG_CATEGORY_GET("decode_alsa_backend");
    ...
    // Needed on device, ignore -s argument.
    state.format = SND_PCM_FORMAT_S16_BE;
#endif
    ...
    if ((err = uname(&utsname)) < 0) {
        LOG_ERROR(log_beep_main, "uname failed: %s", snd_strerror(err));
        exit(-1);
    }
    ...
    if ((err = snd_output_stdio_attach(&output, stdout, 0)) < 0) {
        LOG_ERROR(log_beep_main, "Output failed: %s", snd_strerror(err));
        exit(-1);
    }
    ...
    //printf("Nice ret: %d\n", ret);
    //if (ret == -1 && errno != 0) {
    //    printf("nice failed, %s", strerror(errno));
    //    exit(1);
    //}
