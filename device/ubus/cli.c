...
static int handle_notification(struct ubus_context *ctx, struct ubus_object *obj,
...
static void handle_remove(struct ubus_context *ctx, struct ubus_subscriber *obj,
        uint32_t id) {
    printf("Subscription to %08x ended\n", id); 
    uloop_end();
}

static struct ubus_subscriber subscriber = {
    .cb = handle_notification,
    .remove_cb = handle_remove
};
...
static int usage(const char *prog)
{
	fprintf(stderr,
		...
		" - send <event> [<message>]		Send an event\n"
        " - subscribe <path>                    Subscribe to an object\n"
		"\n", prog);
	return 1;
}
