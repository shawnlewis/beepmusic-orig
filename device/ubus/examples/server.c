...
static int test_shout(struct ubus_context *ctx, struct ubus_object *obj,
		      struct ubus_request_data *req, const char *method,
		      struct blob_attr *msg)
{
	return ubus_notify(ctx, obj, "shout", msg, -1);
}

static void test_subscribe_cb(struct ubus_context *ctx, struct ubus_object *obj)
{
	fprintf(stderr, "Subscribers active: %s\n", (obj->has_subscribers == 1 ? "Yes" : "No"));
}

static const struct ubus_method test_methods[] = {
	UBUS_METHOD("hello", test_hello, hello_policy),
	UBUS_METHOD("watch", test_watch, watch_policy),
	UBUS_METHOD_NOARG("shout", test_shout),
};
...
static struct ubus_object test_object = {
	...
	.subscribe_cb = test_subscribe_cb,
};
