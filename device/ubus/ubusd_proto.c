			...
			uint8_t type)
{
	...
	if (!ub) {
		UBUS_TRACE("!!! ubus_reply_from_blob failed");
		return;
	}
...
static int ubusd_handle_add_object(struct ubus_client *cl, struct ubus_msg_buf *ub, struct blob_attr **attr)
{
	...
	if (!obj) {
		UBUS_TRACE("!!! ubusd_create_object failed");
		return UBUS_STATUS_INVALID_ARGUMENT;
	}
...
static int ubusd_handle_invoke(struct ubus_client *cl, struct ubus_msg_buf *ub, struct blob_attr **attr)
{
	...
	if (!id) {
		UBUS_TRACE("handle_invoke: id not found: %d\n",
    ...
    if (id->id != 1) {
        UBUS_TRACE("--> handle_invoke: cl->id: %u to_id: %u name: %s "
                "seq: %d method: %s params: %s\n",
                cl->id.id,
                id->id,
                (char*) obj->path.key,
                ub->hdr.seq,
                (char*) blob_data(attr[UBUS_ATTR_METHOD]),
                attr[UBUS_ATTR_DATA] ?
                    blobmsg_format_json(attr[UBUS_ATTR_DATA], 1)
                    : "(none)");
    }
...
static int ubusd_handle_response(struct ubus_client *cl, struct ubus_msg_buf *ub, struct blob_attr **attr)
{
    ...
    UBUS_TRACE("<-- handle_response: cl->id: %u from_id: %u name: %s seq: %d status: %d\n",
            cl->id.id,
            obj->id.id,
            (char*) obj->path.key,
            ub->hdr.seq,
            (ub->hdr.type == UBUS_MSG_STATUS) ?
                blob_get_u32(attr[UBUS_ATTR_STATUS]) : -1);
...
void ubusd_proto_receive_message(struct ubus_client *cl, struct ubus_msg_buf *ub)
{
	...
	UBUS_TRACE("--> receive_message: cl->id: %u seq: %d type: %d\n",
	        cl->id.id,
	        ub->hdr.seq,
	        ub->hdr.type);
	...
	ubus_msg_init(retmsg, UBUS_MSG_STATUS, ub->hdr.seq, ub->hdr.peer);

	ubus_msg_free(ub);

	ubus_msg_send(cl, retmsg, true);
}
