...
bool ubus_complete_request_allowed = true;
				...
				struct blob_attr *msg, int cmd, uint32_t peer)
{
    ...
    if (cmd == UBUS_MSG_INVOKE) {
        UBUS_TRACE("--> invoke: local_id: %u to_id: %u seq: %u\n",
                ctx->local_id, peer, req->seq);
    } else if (cmd == UBUS_MSG_ADD_OBJECT) {
        UBUS_TRACE("--> add_object: local_id: %u to_id: %u seq: %u\n",
                ctx->local_id, peer, req->seq);
    }
...
int ubus_complete_request(struct ubus_context *ctx, struct ubus_request *req,
			  int timeout)
{
    if(!ubus_complete_request_allowed) {
        fprintf(stderr, "Not allowed to use ubus_complete_request\n");
        abort();
    }
...
{
	UBUS_TRACE("--> complete_deferred: local_id: %u to_id %u seq: %u ret: %d\n",
            ctx->local_id, req->peer, req->seq, ret);
    blob_buf_init(&b, 0);
		    ...
		    struct blob_attr *msg)
{
    ...
    UBUS_TRACE("--> send_reply: local_id: %u to_id: %u seq: %u\n",
            ctx->local_id, req->peer, req->seq);
	blob_buf_init(&b, 0);
	blob_put_int32(&b, UBUS_ATTR_OBJID, req->object);
...
// De-allocates info and returns the result via the completion callback.
void ubus_invoke_async_finalize(
        struct ubus_invoke_async_info* info, int ret,
        struct blob_attr* response) {
    if (info->has_timeout) {
        uloop_timeout_cancel(&info->timeout);
    }
    if (info->has_lookup) {
        ubus_abort_request(info->lookup_request.req.ctx,
                &info->lookup_request.req);
    }
    if (info->has_request) {
        ubus_abort_request(info->lookup_request.req.ctx, &info->request);
    }
    free(info->msg);
    free(info->method);

    if (ret != -2) {
        info->cb(ret, response, info->priv);
    }
    free(info->response);
    free(info);
}

static void ubus_invoke_async_data_cb(struct ubus_request *req, int type,
        struct blob_attr *msg) {
    struct ubus_invoke_async_info* info = container_of(
            req, struct ubus_invoke_async_info, request);
    free(info->response);
    info->response = blob_memdup(msg);
}

static void ubus_invoke_async_complete_cb(
        struct ubus_request *req, int ret) {
    struct ubus_invoke_async_info* info = container_of(
            req, struct ubus_invoke_async_info, request);
    info->has_request = false;
    ubus_invoke_async_finalize(info, ret, info->response);
}

static void ubus_invoke_async_lookup_cb(struct ubus_context *ctx,
        int ret, struct ubus_object_data *obj, void *priv) {
    struct ubus_invoke_async_info* info = priv;

    info->has_lookup = false;

    if (ret) {
        ubus_invoke_async_finalize(info, ret, NULL);
    } else {
        ret = ubus_invoke_async(
                ctx, obj->id, info->method, info->msg, &info->request);

        if (ret) {
            ubus_invoke_async_finalize(info, ret, NULL);
        } else {
            info->has_request = true;
            info->request.data_cb = ubus_invoke_async_data_cb;
            info->request.complete_cb = ubus_invoke_async_complete_cb;
            ubus_complete_request_async(ctx, &info->request);
        }
    }
}

static void ubus_invoke_async_timeout_cb(struct uloop_timeout *t) {
    struct ubus_invoke_async_info* info = container_of(
            t, struct ubus_invoke_async_info, timeout);
    info->has_timeout = false;
    ubus_invoke_async_finalize(info, UBUS_STATUS_TIMEOUT, NULL);
}

struct ubus_invoke_async_info *ubus_invoke_async_full(
    ...
    // Initialize our info structure
    struct ubus_invoke_async_info* info = calloc(
            1, sizeof(struct ubus_invoke_async_info));
    info->method = strdup(method);
    if(msg) {
        info->msg = blob_memdup(msg);
    }
    info->cb = callback;
    info->priv = priv;
    if (timeout_msecs) {
        info->has_timeout = true;
        info->timeout.cb = ubus_invoke_async_timeout_cb;
        uloop_timeout_set(&info->timeout, timeout_msecs);
    }

    // Perform the object lookup asynchronously.
    int ret = ubus_lookup_async(ctx, path,
            ubus_invoke_async_lookup_cb, info,
            &info->lookup_request);
    if (ret) {
        ubus_invoke_async_finalize(info, ret, NULL);
        return NULL;
    } else {
        // We have an outstanding lookup request
        info->has_lookup = true;
    }
...
void __hidden ubus_process_req_msg(struct ubus_context *ctx, struct ubus_msghdr *hdr)
{
    ...
    UBUS_TRACE("<-- req_message: local_id: %u from_id: %u seq: %u type: %d\n",
            ctx->local_id, hdr->peer, hdr->seq, hdr->type);
