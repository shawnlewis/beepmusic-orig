...
typedef void (*ubus_lookup_handler_t)(struct ubus_context *ctx,
				      ...
				      void *priv);
typedef void (*ubus_lookup_async_handler_t)(struct ubus_context *ctx,
				      int ret,
				      struct ubus_object_data *obj,
				      void *priv);
...
typedef void (*ubus_notify_complete_handler_t)(struct ubus_notify_request *req,
					       int idx, int ret);
typedef void (*ubus_add_object_handler_t)(struct ubus_context *ctx,
					       struct ubus_object *obj, void *priv);
typedef void (*ubus_remove_object_handler_t)(void *priv);
typedef void (*ubus_invoke_async_handler_t)(
        int ubus_ret, struct blob_attr* response, void* priv);
typedef void (*ubus_register_event_handler_handler_t)(void *priv);
...
struct ubus_lookup_async_request {
	struct ubus_request req;
	struct ubus_object_data obj;
	ubus_lookup_async_handler_t cb;
};

struct ubus_invoke_async_info {
    char* method;
    ...
    bool has_lookup;
    struct ubus_lookup_async_request lookup_request;
    ...
    ubus_invoke_async_handler_t cb;
    void *priv;
};
...
/* asynchronous version of add object */
int ubus_add_object_async(struct ubus_context *ctx, struct ubus_object *obj,
		ubus_add_object_handler_t cb, void *priv);

/* remove the object from the ubus connection */
int ubus_remove_object(struct ubus_context *ctx, struct ubus_object *obj);

/* asynchronous version of remove object */
int ubus_remove_object_async(struct ubus_context *ctx, struct ubus_object *obj,
		ubus_remove_object_handler_t cb, void *priv);
...
/* asynchronous ubus_invoke, with timeout and lookup
 Calls callback with ubus_ret if ubus error, or ubus_ret == 0 and response upon
 success. Caller does not own response and should not free it. */
...
/* free a ubus_invoke_async_info and cancel any requests */
...
static inline int ubus_unregister_event_handler_async(struct ubus_context *ctx,
						struct ubus_event_handler *ev,
                        ubus_remove_object_handler_t cb, void *priv)
{
    return ubus_remove_object_async(ctx, &ev->obj, cb, priv);
}
#endif
