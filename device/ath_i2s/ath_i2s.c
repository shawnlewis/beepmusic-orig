...
#include <linux/sched.h>   // Now required by linux/wait.h
...
// Shawn: From arch/mips/atheros/misc.c
/*
 * Reset function
 */
...
int ath_i2s_open(struct inode *inode, struct file *filp)
{
    ...
    if (!opened) {
        ...
        // Also reset sample counts
        ath_reg_wr(ATH_STEREO_TX_SAMPLE_CNT_MSB, 0);
        ath_reg_wr(ATH_STEREO_TX_SAMPLE_CNT_LSB, 0);
...
eagain:
    ...
    // of this function.
    written_samples += count / (num_channels * i2s_word_bytes);
...
void ath_i2s_clk(unsigned long frac, unsigned long posedge)
{
    /*
     * Tick...Tick...Tick
     */
    ...
    //ath_reg_rmw_set(ATH_STEREO_CONFIG, (ATH_STEREO_CONFIG_PSEDGE(0x02) | ATH_STEREO_CONFIG_RESET));
    //printk("CLOCK delay\n");
    //udelay(1000000);
    //printk("CLOCK posedge 1\n");
    //ath_reg_rmw_clear(ATH_STEREO_CONFIG, ATH_STEREO_CONFIG_PSEDGE(0xff));
    ...
    ath_reg_wr(ATH_STEREO_CONFIG, rddata);

    //ath_reg_rmw_set(ATH_STEREO_CONFIG, ATH_STEREO_CONFIG_RESET);
    //ath_reg_rmw_clear(ATH_STEREO_CONFIG, ATH_STEREO_CONFIG_RESET);
    //udelay(1000000);
}
EXPORT_SYMBOL(ath_i2s_clk);

// To derive mck/mclk (this is the i2s clock):
//    mclk = 400mhz / ((DIV_INT + DIV_FRAC) * POSEDGE)
// Example:
//    DIV_INT = 0x11 = 17
//    DIV_FRAC = 0xb725 = 0.7161
//    POSEDGE = 2
//    mclk = 400mhz / ((17 + 0.7161) * 2) = 11.2892
//
// Or:
//    mclk = sample_rate * 32bits/word * 2stereo * 4 (why 4?)
//
// Best approach is to calculate what you want mclk to be based on
// the second equation, then use the first equation to figure out values
// for DIV and POSEDGE.
int ath_i2s_set_freq(uint32_t data)
{
    switch (data) {
    case 22050:
        ath_i2s_clk(0x11b725, 4);
        break;
    case 32000:
        ath_i2s_clk(0x180000, 2);
        break;
    case 44100:
        ath_i2s_clk(0x11b725, 2);
        break;
    case 48000:
        // assumes division is based on 400MHz cpu clock.
        ath_i2s_clk(0x208d54, 1);
        break;
    default:
...
// Returns the number of samples the driver has played.
// We need to check for rollover since the sample count is split across
// two registers.
// This needs to take a constant amount of time to execute for really
// accurate sync. But does it?
inline uint32_t get_played_samples(void) {
    uint32_t lsb, lsb2, msb;
    lsb = ath_reg_rd(ATH_STEREO_TX_SAMPLE_CNT_LSB) & 0xffff;
    msb = ath_reg_rd(ATH_STEREO_TX_SAMPLE_CNT_MSB) & 0xffff;
    lsb2 = ath_reg_rd(ATH_STEREO_TX_SAMPLE_CNT_LSB) & 0xffff;
    //printk("lsb: %u msb: %u lsb2: %u\n", lsb, msb, lsb2);
    if (lsb2 < lsb) {
        // rolled over
        msb++;
    }
...
long ath_i2s_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    int data;
    ...
    case I2S_CLEAR_OUT_SAMPLE_COUNT:
        sample_count = (uint32_t*) arg;
        // TODO: Could do this atomically by using clear on read sample
        // clearing strategy. But then we'd have to track samples. The
        // timing should be deterministic, so the slight offset should
        // be consistent across devices (but if we want to sync with
        // other devices/drivers we should fix this).

        // Returns the sample count before the clear for channel 0.
        if (sample_count) {
            *sample_count = get_played_samples();
        }
        ath_reg_wr(ATH_STEREO_TX_SAMPLE_CNT_MSB, 0);
        ath_reg_wr(ATH_STEREO_TX_SAMPLE_CNT_LSB, 0);
    ...
    case I2S_GET_SYNC:
        i2s_sync = (struct i2s_sync*) arg;
        // TODO(shawn): We should probably use the monotonic clock.
        do_gettimeofday(&i2s_sync->tv);
        i2s_sync->samples = get_played_samples();
        i2s_sync->buffered_samples = written_samples - i2s_sync->samples;
        return 0;
...
void ath_i2s_request_dma_channel(int mode)
{
    ath_reg_wr(ATH_MBOX_DMA_POLICY,
                    (ATH_MBOX_DMA_POLICY_RX_QUANTUM |
                     ATH_MBOX_DMA_POLICY_RX_QUANTUM |  // NOTE: Bug? TX
