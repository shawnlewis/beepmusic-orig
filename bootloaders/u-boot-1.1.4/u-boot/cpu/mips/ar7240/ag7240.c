...
#if(CONFIG_COMMANDS & CFG_CMD_MII)
// The prototypes for miiphy_register are completely different than the
// ag7240_miiphy_read/write.  Use shims to connect the two as the
// Atheros functions are used elsewhere in u-boot.

// Per the comments in common/miiphyutil.c return 0 for success.
int __ag7240_miiphy_read(char *devname, unsigned char addr, unsigned char reg,
        unsigned short *value) {
    uint16_t ret = ag7240_miiphy_read(devname, (uint32_t)addr, (uint8_t)reg);
    *value = (unsigned short)ret;
    return 0;
}

int __ag7240_miiphy_write(char *devname, unsigned char addr, unsigned char reg,
        unsigned short value) {
    ag7240_miiphy_write(devname, (uint32_t)addr, (uint8_t)reg, (uint16_t)value);
    return 0;
}
#endif
...
int ag7240_enet_initialize(bd_t * bis)
{
    ...
    for (i = 0;i < CFG_AG7240_NMACS;i++) {
        eth_register(dev[i]);
#if(CONFIG_COMMANDS & CFG_CMD_MII)
        miiphy_register(dev[i]->name, __ag7240_miiphy_read,
                __ag7240_miiphy_write);
#endif
        ...
        {
            unsigned char *mac __attribute__((unused)) = dev[i]->enetaddr;
