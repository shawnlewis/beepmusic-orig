...
#if defined(CONFIG_SHOW_ACTIVITY) && defined(CONFIG_BOARD_CARAMBOLA2)
...
#else
#define usbboot_indicate_recovery(TYPE)
#endif
...
int usbboot_recovery(char * boot_dev_part, char * boot_file_name)
{
	...
	if (fat_read_ret > 0){
		...
		do_run ( cmdtp,  0,  argc_flash, argv_flash);
...
int usbboot_scan(cmd_tbl_t *cmdtp, int flag, int argc, char * argv[])
{
	int i;
	char part_id[8];
