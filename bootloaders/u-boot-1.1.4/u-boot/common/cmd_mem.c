...
{
	ulong	addr, length, i;
    ulong junk __attribute__((unused));
	int	size;
	volatile uint	*longp;
...
#else  // !defined(CFG_ALT_MEMTEST)
	if(argc > 3){
		max_iterations = (ulong)simple_strtoul(argv[3], NULL, 10);
		if(max_iterations > INT_MAX){
			max_iterations = 0;
		}
	} else {
		max_iterations = 0;
...
#if defined(CFG_ALT_MEMTEST)
	"mtest   - complete, alternative RAM test\n",
	"[start [end [max iter]]]\n"
	"    - complete, alternative RAM read/write test\n"
#else  // defined(CFG_ALT_MEMTEST)
	"mtest   - simple RAM test\n",
	"[start [end [pattern]]]\n"
	"    - simple RAM read/write test\n"
#endif  // defined(CFG_ALT_MEMTEST)
);
