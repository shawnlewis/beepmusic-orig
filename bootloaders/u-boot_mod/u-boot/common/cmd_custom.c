...
int do_bootb(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[]) {
    ...
    if (test_mode_gpio_status()) {
        // Call mtest.
        cmd_argv[0] = "mtest";
        cmd_argv[1] = "0x80200000";  // mem start
        cmd_argv[2] = "0x83dfffff";  // mem end
    ...
    if (boot_primary && reset_button_status()) {
        while (reset_button_status() && counter < RESET_BUTTON_MAX_COUNT) {
            milisecdelay(RESET_BUTTON_MS_DELAY);
            counter++;
        }
    }

    if (counter >= RESET_BUTTON_MAX_COUNT) {
        boot_primary = 0;
    }
...
U_BOOT_CMD(bootb, 1, 0, do_bootb, "boot Beep image\n", NULL);
#endif  // defined(CONFIG_FOR_BEEP_CM2) || defined(CONFIG_FOR_BEEP01)
